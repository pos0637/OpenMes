package com.furongsoft.rbac.filters;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private void responseUnauthorized(ServletRequest req, ServletResponse resp) {
        ...
        try {
            ...
            response.setContentType(MediaTypes.HAL_JSON_UTF8_VALUE);

            out = response.getWriter();
            out.append(StringUtils.toHALString(new RestResponse(HttpStatus.UNAUTHORIZED, "invalid token", null)));
            out.flush();
        } catch (IOException e) {
            Tracker.error(e);
        } finally {
